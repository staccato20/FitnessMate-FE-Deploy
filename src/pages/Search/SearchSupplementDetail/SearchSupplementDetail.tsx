...
const SearchSupplementDetail = () => {
  ...
  const handleAdd = () => {
    ...
    TokenApi.post(`/myfit/routines/supplement`, { supplementIds: submission })
      .then((response) => {
  ...
  return (
    <S.SearchWorkoutContainer>
      <S.BeforeButton onClick={handleBackPage}>
        ...
        />
        <span className="backBtnText">이전</span>
      </S.BeforeButton>
      <S.TopContainer>
        <S.TopLeft>
          <S.SpplementName>{supplementName}</S.SpplementName>
        </S.TopLeft>
        <button onClick={handleAdd}>내 보조제에 추가</button>
      </S.TopContainer>
      <S.MiddleContainer>
        <img
          src={imgPath}
          alt={`${supplementName} 보조제 이미지`}
        />
        <S.InformationContainer>
          ...
          <button onClick={() => window.open(marketURL, "_blank")}>
            구매하러 가기
          </button>
        </S.InformationContainer>
      </S.MiddleContainer>
