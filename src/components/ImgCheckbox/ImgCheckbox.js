...
import * as ImgCheck from "./StyledImgCheckbox";


// article : 추천받는 항목이 운동과 보조제가 있어서 우선 분리했으나, 
// 추후 백과 합치면서 체크박스 onClick 애니메이션 제작에 영향을 받아 수정될 가능성 있음
// 위 문제와 더불어 back 코드와 함께 다룰 필요가 있는 체크박스 애니메이션 제작은 하지 않았음

// articleimg : 추천받는 항목인 운동과 보조제 각각의 이미지를 첨부
// 파일명만 작성하여 불러올 수 있으며, 확장자는 png만 가능

function ImgCheckbox({ article, articleimg }) {
	return (
		<ImgCheck.StyledImgCheckbox>
			<div className="ImgCheckbox-top">
				<div>
					<span className="recommend-article">{article} </span><span className="recommend-text">추천 받을래요</span>
				</div>
